...
predictions2 = zeros(size(train,1),1);
    for m = 1:size(train,1)
        [~,predictions2(m)] = max(train_scores(m,:));
    end

accuracy = 100*sum(predictions==test(:,end))/size(test,1);
disp(['Accuracy = ' num2str(accuracy) ' %']);

accuracy2 = 100*sum(predictions2==train(:,end))/size(train,1);
disp(['Training Accuracy = ' num2str(accuracy2) ' %']);